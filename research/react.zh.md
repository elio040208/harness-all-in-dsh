...
- 公共 Harness 参考：[JIT-Agent](https://arxiv.org/abs/2608.25593)和 HarnessFactory revision `ababa06c2f54d799fd9fbc356e5368f61a452260`。

## 定义行为

官方 HotpotQA loop 保留完整 prompt 轨迹，并反复要求模型先生成 `Thought`，再生成一个 `Action`。环境执行 `Search`、`Lookup` 或 `Finish`，追加对应 `Observation`，最多重复七轮。固定 Harness 解释使用 FullHistory memory、不使用显式 planner、执行 ReAct action，并开放完整任务能力目录。
