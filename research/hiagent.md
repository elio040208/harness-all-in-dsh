...
- Inspected JIT files: `harness_factory/harnesses/hiagent/{memory.py,action.py,planning.py,tool_policy.py,prompt.yaml}` and `harness_factory/descriptions/hiagent.md`.
...
- The JIT adaptation permits five tools in one step. The official `OurAgent` emits one action, so the DSH profile keeps one external call per step.
