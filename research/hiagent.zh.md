...
- JIT HarnessFactory revision `ababa06c2f54d799fd9fbc356e5368f61a452260`；检查 HiAgent memory、action、planning、tool policy、prompt 和 description。
...
论文还描述基于 subgoal chunk、observation summary 和详细 trajectory retrieval 的更广义层次设计。这些机制没有出现在官方仓库注册的 `OurAgent` 中，其可执行 memory policy 只有 value-based trajectory sampling。JIT 固定适配沿用该代码路径并称为 `HierarchicalMemory`。
...
- JIT 适配允许每步五个工具；官方 `OurAgent` 每步一个 action，因此 headless profile 保持一个外部 call。
