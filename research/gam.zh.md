...
- JIT HarnessFactory revision `ababa06c2f54d799fd9fbc356e5368f61a452260`；检查 GAM memory、planning、action、tool policy、prompt 和 description。
...
每四个 action step，task tool 暂停，模型执行 JIT research。`gam_search_pages` 支持对完整内容进行精确关键词和 page id 检索；`gam_integrate_memory` 保存整合结果及其来源 page id。随后插件把完整非 system surface 替换为包含原始任务和 integrated memory 的用户 checkpoint。原始 Session event 和 page projection 仍可用于回放和后续检索。
